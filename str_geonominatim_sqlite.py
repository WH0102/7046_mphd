...
def getting_str_geolocation(sqlite_db:str) -> None:
    conn = sqlite3.connect(sqlite_db)
    # len_df = pd.read_sql("SELECT COUNT(*) FROM str", conn)
    len_df = 11400627
    # To run the program in loop of 100
    add_loop = 1000
    for num in range(2535000, len_df, add_loop):
        # For monitoring in the terminal
        print(f"{num:,d}")
        # To avoid unnecessary ping
        # time.sleep(1)
        # To slice the dataframe
        temp_df = pd.read_sql(f"SELECT NoKPKIR,address FROM str LIMIT {add_loop} OFFSET {num}", con=conn)
...
if __name__ == "__main__":
    ...
    getting_str_geolocation(r"/Users/wh0102/Downloads/github/7046_mphd/str_address/str.db")